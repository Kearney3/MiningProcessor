...
项目包含 5 个独立的 Excel 处理脚本和一个 Flet 图形界面：
...
| `gui/main.py` | Flet 图形界面 | 集成所有处理模块 |
...
Flet 实现的桌面 GUI（`gui/main.py`），支持：
- 设备台账导入/导出模板
- 设备装载量配置（GUI 可视化编辑）
- 4 个数据处理模块：燃油、生产（enhanced）、电力、工时
- 处理日志实时显示
- 跨平台 macOS/Windows 分发（PyInstaller 打包）
...
| 文件 | 说明 | 状态 |
|------|------|------|
| `gui/main.py` | Flet GUI | 已完成 |
| `config_loader.py` | 配置加载 | 已完成 |
| `equipment_ledger.py` | 设备台账模块 | 已完成（待启用） |
| `config.json` | 配置文件 | 已完成 |
| `pyproject.toml` | 依赖管理（Flet） | 已完成 |
| `excel_worktime.py` | 工时/矿卡处理 | 已有，待集成配置 |
| `excel_fuel.py` | 燃油处理 | 已有 |
| `excel_electrical.py` | 电力处理 | 已有 |
| `excel_production_enhanced.py` | 增强版生产处理 | 已有 |
| 模糊匹配集成 | 多表合并时启用 | 待实现 |