...
核心依赖：`pandas` + `openpyxl` 负责 Excel 解析，`xlsxwriter` 负责高性能导出，`flet` 负责 GUI，`rapidfuzz` 用于设备名称模糊匹配，`xlrd` 支持旧版 `.xls` 格式读取。
...
测试覆盖 233 个用例，涵盖 GUI 组件、配置读写、日志分发、Excel 合并、设备台账、油品台账等多个模块（详见 README.md 测试章节）。
...
- 测试分布在 `tests/` 下 14 个文件中（233 个用例），核心文件包括：
