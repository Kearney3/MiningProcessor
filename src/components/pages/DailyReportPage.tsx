...
export function DailyReportPage({ bridge }: { bridge: BridgeProp }) {
  ...
  const exportReport = async () => {
    ...
    setLoading(true);
    try {
      const result = await bridge.call<{ output_file: string; rows: number; warnings: Record<string, unknown>[] }>("daily_report_export", {
      ...
      notify(`日报已保存：${result.rows} 行，警告 ${result.warnings.length} 条`, result.warnings.length ? "info" : "success");
    } catch (error) {
