<p align="center">
  <img src="src-tauri/icons/icon.png" alt="MiningProcessor 项目图标" width="144" />
</p>
...
> 一套处理引擎，两种使用方式：CLI 与 Tauri 桌面应用。
...
| `excel_production_enhanced.py` | `production` | 增强版生产报表解析（Tauri 默认） | 生产报表文件/文件夹 | 生产数据汇总 |
...
│   ├── excel_production_enhanced.py # 生产报表（Tauri 默认）
...
│       └── constants.py
├── tauri_bridge.py             # JSON-RPC over stdio 服务端（Tauri RPC 入口）
├── tauri_bridge.spec           # PyInstaller 打包配置
├── public/fonts/MiSansVF.ttf   # 字体资源
├── src-tauri/icons/            # Tauri 图标与 logo 源文件
...
> **⚠️ 行为说明**：Tauri 应用中"应用当前配置"仅更新运行时内存（`apply_device_load_map()`），"保存配置"才会写回文件（`update_device_load_map()`）。
...
- **展示层**（`src/`）：React + TypeScript 页面负责 UI 控件、用户输入和日志渲染，不含 Excel 业务逻辑。
- **桥接层**（`src-tauri/` + `tauri_bridge.py`）：Rust 管理 Python sidecar 生命周期；`tauri_bridge.py` 实现 JSON-RPC over stdio 协议。
- **业务层**（`func/`）：Excel 解析、数据提取、配置管理、日志。CLI 与 Tauri 共享同一套处理逻辑。
...
2. 在 `src/components/pages/` 增加或更新 React 页面；
3. 在 `tauri_bridge.py` 注册 RPC 方法，并通过 `func/` 调用处理函数。

### 统一日志

`func/logger.py` 提供 `logging` + `get_logger()`，CLI 直接输出控制台。Tauri 将包含序号、时间、来源和异常详情的结构化事件从 Python stderr 经 Rust 有界通道转发，React 再按 50ms 窗口合并渲染。新增处理逻辑请使用 `logging` 而非 `print()`。

Tauri 日志面板有界保留日志，用户上翻时暂停自动跟随；队列高峰时优先保留 WARNING/ERROR。界面只显示异常根因，诊断日志保留完整 traceback。
...
| `test_logic_helpers.py` | 编排、输出路径与台账后处理辅助函数 |
| `test_excel_merger.py` / `test_table_merge.py` | Excel 合并与表内合并聚合 |
| `test_logger.py` | 日志格式化 |
| `test_ledger_mapping.py` | 设备台账匹配与后缀 |
...
Tauri 前端测试在 `src/test/` 下，使用 Vitest + Testing Library，覆盖页面、共享组件、国际化与 Python bridge。
...
详见 `.github/workflows/build-tauri.yml`。
...
在 GitHub Actions 页面选择 `Build Tauri App` workflow，点击 `Run workflow`。

### 版本号管理

项目版本号统一由 `scripts/bump_version.py` 管理，以 `pyproject.toml` 为唯一真相源，自动同步到：

- `package.json`（Node.js / Tauri 前端）
...
- 🧩 **双端与台账集成**
  - Tauri 接入轮胎处理，支持安装车辆与设备台账匹配、跳过隐藏行和隐藏列
...
- 🏗️ **架构重构：Python 统一处理编排层**
  - `process_single()` 成为单报表处理的统一调度入口，集中模块调用与后处理
  - `get_output_path()` 统一 6 种模块的输出路径计算，避免输出路径逻辑重复
...
- 🆕 油耗模块新增"过滤零小时数"和"过滤零运行小时数"开关：发动机小时数或运行小时数为 0 / 空时自动过滤（Tauri / CLI）
- 🆕 生产模块运行数据新增 4 个过滤开关：过滤零小时仪表、零公里仪表、零运行小时数、零运行里程（Tauri / CLI）
...
- 🆕 油耗模块新增"过滤零小时数"开关（Tauri / CLI）：勾选后发动机小时数开始或结束为 0 或为空的记录将被过滤
- 🆕 油耗模块新增"过滤零运行小时数"开关（Tauri / CLI）：勾选后运行小时数为 0 或为空的记录将被过滤
- 🎨 两个新开关位于油耗模块卡片内部，而非全局选项区，更符合模块专属配置的直觉

### v2.0.1 · 2026-07-28

- 🆕 文件合并新增"兼容表头"开关（Tauri / CLI `--tolerant-header`）：勾选后表头不一致的文件也能合并，缺失列自动填空；未勾选时保持原有严格校验行为
...
- 🆕 新增字符级 TF-IDF + 分层线性分类器，对规则仍判为“其他”的故障记录进行高置信度二级识别；规则结果始终优先
- 🆕 Tauri 和 CLI 均支持启用/关闭机器学习辅助识别，维修明细增加“分类方式”和“分类置信度”
...
- 🆕 桌面安装包内置当前生效模型；Tauri 和 CI 构建流程增加模型存在性、可加载性及打包完整性检查
...
- 🆕 页面关闭时终止后台任务：关闭 Tauri 窗口自动取消正在运行的批量处理任务，生产数据处理支持中途取消（cancel_event 透传到多线程处理循环）
- 🆕 批量 / 生产处理汇总 UI：处理完成后展示成功/失败模块列表及警告信息（如未匹配装载量型号）
- 🆕 逐列异常检测开关持久化：从各页面运行时 UI 移除，统一迁移到用户配置页面集中管理，所有 GUI 入口从 `config_loader` 读取，配置更清晰
- 🆕 Tauri 用户配置页面增强：新增逐列异常检测开关管理界面
